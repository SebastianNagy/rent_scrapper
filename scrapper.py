...
class Scrapper:
    def __init__(self, ID, URL, findall_kwargs, extra_regex=None):
        self.browser = None
        self.ID = ID
        self.URL = URL
    ...
    def scrapper_loop(self):
        ...
        while self.last_known_count is None:
            ...
            if self.last_known_count is not None:
                ...
                telegram_bot.send_telegram_message(f"🔎 {self.ID} Scraper started. Initial rental count is {self.last_known_count}.")
            else:
            ...
            try:
                ...
                if current_count is not None:
                    ...
                    if self.last_known_count is not None and current_count != self.last_known_count:
                        ...
                        self.last_known_count = current_count
                        telegram_bot.send_telegram_message(f"✅ {self.ID} Scraper successfully fetched data. Current count is {current_count}.")
            except KeyboardInterrupt:
                log(f"[{self.ID}] Scraper stopped by user.")
                break
            except Exception as e:
                log(f"An error occurred in the main loop: {e}")
                telegram_bot.send_telegram_message(f"❌ {ID} Scraper crashed: {e}")
